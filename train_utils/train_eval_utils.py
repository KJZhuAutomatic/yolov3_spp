...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with amp.autocast(enabled=scaler is not None):
            ...
            shape [n, num_cls] and cls shape [n,]. To specify the point value 1, 
            tgt_cls[range(n), cls] = 1 is correct. See yolov3_spp.compute_loss
            for i in model.yolo_layers:
                model.module_list[i].anchor_vec = model.module_list[i].anchor_vec.to(device) 
            pred_clone = [p.clone() for p in pred]
